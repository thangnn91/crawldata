...
def click_to_element(driver, city, district):
    ...
    for e in districts:
        if e.text.find(district) != -1:
            driver.execute_script("arguments[0].click();", e)
            break


def connect_db():
    return mysql.connector.connect(user='developer', password='sunny63811', host='14.162.60.191', database='djangodb_crawl')
...
for data in list_data:
    ...
    # base_url = 'https://muaban.net/mua-ban-nha-dat-cho-thue-quan-ba-dinh-l2406-c3'
            ...
            for item in links_hrefs:
                ...
                try:
                    ...
                    content_text = block_article_detail.find_element_by_class_name(
                        'body-container').text
                    article["description"] = content_text.encode(
                        'unicode_escape')
                    # publisher mobile
                    try:
